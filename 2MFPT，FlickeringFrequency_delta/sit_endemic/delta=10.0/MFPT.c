...
{	
	...
	// a1=7.5+(filenum-1.0)*0.5;
	a1=10.0;
	
	       	x2[0] = 1;x2[1] = 0;x2[2] = 1;x2[3] = 0, x2[4] = 0;x2[5] = 0,x2[6] = 0; //无病态 
	       	x1[0] = 0.14830;x1[1] = 0.20270;x1[2] = 0.18910;x1[3] = 0.07210;x1[4] = 0.19020;x1[5] = 0.32680,x1[6] = 0.07480; //有病态 
...
void force(double xp[], double fvec[]) 
{
	...
	fvec[0] = q*xp[1] - p*(a1)*xp[3];                                                       
	fvec[1] = r*(1-xp[0]-xp[1]) - q*xp[1] ;              
	fvec[2]=q*xp[5] + w*xp[0]*xp[3]/(xp[0]+xp[1]) - 2*p*(a1)*(xp[2]*xp[3])/xp[0];    
	fvec[3]=2*p*(a1)*(xp[2]*xp[3])/xp[0] + q*xp[6] - r*xp[3] - w*xp[3] - p*(xp[3]+(a1)*xp[3]*xp[3]/xp[0]); 
	fvec[4]=p*(xp[3]+(a1)*xp[3]*xp[3]/xp[0]) - 2*r*xp[4];
	fvec[5]=r*xp[3] + w*xp[1]*xp[3]/(xp[0]+xp[1]) + 2*q*(1-xp[2]-xp[3]-xp[4]-xp[5]-xp[6]) - q*xp[5] - p*(a1)*xp[3]*xp[5]/xp[0] + w*xp[0]*xp[6]/(xp[0]+xp[1]);
	fvec[6]=2*r*xp[4] + p*(a1)*xp[3]*xp[5]/xp[0] - q*xp[6] - r*xp[6] - w*xp[6] ;	  
